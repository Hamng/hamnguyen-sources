...
Edit an array of keypress chars to process r"\b" by removing it
and the preceding char.
Notes:
    1.  This is NOT the single backspace char ('\b'), but 2 consecutive
        chars: a backslash char ('\') followed by a 'b'
    2.  Must handle multiple occurences of "\b" (of course).
    3.  Must handle (possibly multiple) r"\b" at the beginning of the array.
    4.  Must handle "excessive \b"; i.e. more r"\b" than avail chars.
...
def delete_backspace(s):
    ...
    non_lead_bs = re.compile(r"""
                                 ...
                                 (.*)   # group \1: rest
                             """, re.VERBOSE)   # to document pattern
    s, nsubs = re.subn(non_lead_bs, r"\1", s, count=1)  # do only 1 sub
    #print("-char+bs<%s>" % s)
    while nsubs > 0:
        s = re.sub(leading_bs, r"\2", s)
        #print("-lead_bs<%s>" % s)
